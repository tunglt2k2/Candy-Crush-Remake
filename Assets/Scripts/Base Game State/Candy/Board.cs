...
public class Board : MonoBehaviour
{
    ...
    public GameObject slimeTilePrefabs;
    ...
    private void GenerateSpecialTiles()
    {
        for (int i = 0; i < boardLayout.Length; i++)
        {
            ...
            {
                GameObject tile = Instantiate(slimeTilePrefabs, tempPosition, Quaternion.identity);
        ...
        for (int i = 0; i < width; i++)
            ...
            {        
                if (!blankSpaces[i, j])
                {
                    ...
                    GameObject backgroundTile = Instantiate(tilePrefap, tempPosition, Quaternion.identity) as GameObject;
                    ...
                    if(!concreteTiles[i, j] && !slimeTiles[i,j])
                    {
                        ...
                        //Reload when apear matched
                        List<int> numDot = new List<int>();
                        while (MatchesAt(i, j, dots[dotToUse]))
                        {
                            numDot.Add(dotToUse);
                            dotToUse = RandomExcept(dots.Length, numDot);
                        }
                        numDot.Clear();
    ...
    private int RandomExcept(int n, List<int> list) //Random from 0 to n-1 except list int
    {
        list.Sort();
        int result = Random.Range(0, n - list.Count);

        for(int i =0; i< list.Count; i++)
        {
            if(result < list[i]) return result;
            result++;
        }
        return result;
    }
    ...
    private MatchType DecideBomb()
    {
        ...
        for (int i = 0; i < matchCopy.Count; i++)
        {
            ...
            for (int j = 0; j < matchCopy.Count; j++)
            {
                ...
                if (nextDot.column == column && nextDot.tag == color)
                {
                    columnMatch++;
                }
                if (nextDot.row == row && nextDot.tag == color)
                {
                    rowMatch++;
                }
            }
            //Return 3 if colunm or row match
    ...
    {
        //How many objects are in findMatches.Count > 3
        if(findMatches.currentMatches.Count > 3)
        {
            //What type of match?
            MatchType typeOfMatch = DecideBomb();
            ...
            {
                findMatches.CheckBomb(typeOfMatch);
            }
        }
    }
    ...
    public void  DestroyMatchesAt(int column, int row)
    {
        if (allDots[column, row].GetComponent<Dot>().isMatched)
        {
            ...
            GameObject particle = Instantiate(allDots[column,row].GetComponent<Dot>().destroyEff, allDots[column, row].transform.position, Quaternion.identity);
    ...
    private void DamageConcenteTile(int column, int row)
    {
        if(column > 0)
        {
    ...
    private void DamageSlimeTile(int column, int row)
    {
        if (column > 0)
        {
    ...
    private IEnumerator DecreaseRowCo()
    {
        for (int i = 0; i < width; i++)
        {
            ...
            {   //if the current spot isn't blank and is empty
                ...
                {
                    //loop for the space above to the top of the column
    ...
    private bool MathchesOnBoard()
    {
        findMatches.FindAllMatches();
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
                if(allDots[i,j] != null && allDots[i, j].GetComponent<Dot>().isMatched) return true;  
        return false;
    }
    ...
    private Vector2 CheckForAjacent(int column, int row)
    {
    ...
    private void MakeNewSlime()
    {
        ...
        while(!slime && loops < 200)
        {
            ...
            {
                Vector2 adjacent = CheckForAjacent(newX, newY);
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (allDots[i, j] != null)
                {
                    ...
                    if (i < width - 2)
                    {
                        //Check if the dots to the right and two the right
    ...
    private IEnumerator SuffleBoard()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if(allDots[i,j] != null && !breakableTiles[i,j])
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                if(!blankSpaces[i,j] && !concreteTiles[i,j] && !breakableTiles[i,j] && !slimeTiles[i,j])
                {
                    //Pick a random number
                    int pieceToUse = Random.Range(0, newBoard.Count);
                    int maxInterations = 0;
                    while (MatchesAt(i,j,newBoard[pieceToUse]) && maxInterations < 1000)
                    {
